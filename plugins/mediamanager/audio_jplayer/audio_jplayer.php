...
class plgMediaManagerAudio_Jplayer extends MediaManagerPluginBase
{
	public $_element = 'audio_jplayer';
	public $dot_path_prefix = 'audio_jplayer.';
	public $path_prefix = 'audio_jplayer/';
	public $plain_name = 'AudioJplayer'; 
	public $full_path = '';
	public $relative_path = '';
	
	function plgMediaManagerAudio_Jplayer( &$subject, $config )
	{
	...
	public function onDisplayMediaItem( $plugin, $media_item )
	{
        ...
        $vars->width = $media_item->params->get( 'audiojplayer_width', $vars->config->get( 'audio_default_width' ) );
        $vars->height = $media_item->params->get( 'audiojplayer_height', $vars->config->get( 'audio_default_height' ) );
    ...
    public function onEditMedia( $plugin, $media_item )
    {
        ...
        MediaManager::load( 'MediaManagerSelect' . $this->plain_name, $this->dot_path_prefix . 'audio_jplayer.library.select', array( 'site'=>'site', 'type'=>'plugins', 'ext'=>'mediamanager' ) );
        ...
        foreach ($gallery_images as $image)
        {
            if (!in_array($image, $filenames_from_db))
            {
                ...
                if ($file->save())
                {
                    ...
                    if ($mediafile->save())
                    {
                        // and create the corresponding row in the audio_jplayer table
                        $audiojplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                        $audiojplayer_table->media_id = $media_item->media_id;
                        $audiojplayer_table->mediafile_id = $mediafile->mediafile_id;
                        if (!$audiojplayer_table->save())
                        {
                            JFactory::getApplication()->enqueueMessage( $audiojplayer_table->getError(), 'notice' );
                        }
                    }
                        else
                    {
    ...
    public function onAfterSaveMedia( $media_item )
    {
		...
		$params->set('audiojplayer_auto_play', $post['audiojplayer_auto_play']);
		$params->set('audiojplayer_width', $post['audiojplayer_width']);
		$params->set('audiojplayer_height', $post['audiojplayer_height']);
		$params->set('audiojplayer_controlbar_position', $post['audiojplayer_controlbar_position']);
		$params->set('audiojplayer_show_icons', $post['audiojplayer_show_icons']);
		$params->set('audiojplayer_buffer_length', $post['audiojplayer_buffer_length']);
		$params->set('audiojplayer_loop', $post['audiojplayer_loop']);
        ...
        if (!empty($item_remote_new))
        {
            ...
            if ($file->save())
            {
                ...
                if ($mediafile->save())
                {
                    // and create the corresponding row in the audio_jplayer table
                    $audiojplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                    $audiojplayer_table->load( array( 'mediafile_id'=>$mediafile->mediafile_id, 'media_id'=>$media_item->media_id ) );
                    $audiojplayer_table->media_id = $media_item->media_id;
                    $audiojplayer_table->mediafile_id = $mediafile->mediafile_id;
                    $audiojplayer_table->enabled = '1';
                    if (!$audiojplayer_table->save())
                    {
                        JFactory::getApplication()->enqueueMessage( $audiojplayer_table->getError(), 'notice' );
                    }
