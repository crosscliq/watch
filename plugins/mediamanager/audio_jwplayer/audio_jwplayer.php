...
class plgMediaManagerAudio_Jwplayer extends MediaManagerPluginBase
{
	public $_element = 'audio_jwplayer';
	public $dot_path_prefix = 'audio_jwplayer.';
	public $path_prefix = 'audio_jwplayer/';
	public $plain_name = 'AudioJwplayer'; 
	public $full_path = '';
	public $relative_path = '';
	
	function plgMediaManagerAudio_Jwplayer( &$subject, $config )
	{
	...
	public function onDisplayMediaItem( $plugin, $media_item )
	{
        ...
        $vars->width = $media_item->params->get( 'audiojwplayer_width', $vars->config->get( 'audio_default_width' ) );
        $vars->height = $media_item->params->get( 'audiojwplayer_height', $vars->config->get( 'audio_default_height' ) );
        
        $media = JTable::getInstance( 'Media', 'MediaManagerTable' );
        $media->load( $media_item->media_id );
        
        $image = (!empty($media_item->media_image_remote)) ? $media_item->media_image_remote : null;
    ...
    public function onEditMedia( $plugin, $media_item )
    {
        ...
        MediaManager::load( 'MediaManagerSelect' . $this->plain_name, $this->dot_path_prefix . 'audio_jwplayer.library.select', array( 'site'=>'site', 'type'=>'plugins', 'ext'=>'mediamanager' ) );
        ...
        foreach ($gallery_images as $image)
        {
            if (!in_array($image, $filenames_from_db))
            {
                ...
                if ($file->save())
                {
                    ...
                    if ($mediafile->save())
                    {
                        // and create the corresponding row in the audio_jwplayer table
                        $audiojwplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                        $audiojwplayer_table->media_id = $media_item->media_id;
                        $audiojwplayer_table->mediafile_id = $mediafile->mediafile_id;
                        if (!$audiojwplayer_table->save())
                        {
                            JFactory::getApplication()->enqueueMessage( $audiojwplayer_table->getError(), 'notice' );
                        }
                    }
                        else
                    {
    ...
    public function onAfterSaveMedia( $media_item )
    {
		...
		$params->set('audiojwplayer_auto_play', $post['audiojwplayer_auto_play']);
		$params->set('audiojwplayer_width', $post['audiojwplayer_width']);
		$params->set('audiojwplayer_height', $post['audiojwplayer_height']);
		$params->set('audiojwplayer_controlbar_position', $post['audiojwplayer_controlbar_position']);
		$params->set('audiojwplayer_show_icons', $post['audiojwplayer_show_icons']);
		$params->set('audiojwplayer_buffer_length', $post['audiojwplayer_buffer_length']);
		$params->set('audiojwplayer_loop', $post['audiojwplayer_loop']);
        ...
        if (!empty($item_remote_new))
        {
            ...
            if ($file->save())
            {
                ...
                if ($mediafile->save())
                {
                    // and create the corresponding row in the audio_jwplayer table
                    $audiojwplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                    $audiojwplayer_table->load( array( 'mediafile_id'=>$mediafile->mediafile_id, 'media_id'=>$media_item->media_id ) );
                    $audiojwplayer_table->media_id = $media_item->media_id;
                    $audiojwplayer_table->mediafile_id = $mediafile->mediafile_id;
                    if (!$audiojwplayer_table->save())
                    {
