...
class plgMediaManagerVideo_Jplayer extends MediaManagerPluginBase
{
	public $_element = 'video_jplayer';
	public $dot_path_prefix = 'video_jplayer.';
	public $path_prefix = 'video_jplayer/';
	public $plain_name = 'VideoJplayer'; 
	public $full_path = '';
	public $relative_path = '';
	
	function plgMediaManagerVideo_Jplayer( &$subject, $config )
	{
	...
	public function onDisplayMediaItem( $plugin, $media_item )
	{
        ...
        $vars->width = $media_item->params->get( 'videojplayer_width', $vars->config->get( 'video_default_width' ) );
        $vars->height = $media_item->params->get( 'videojplayer_height', $vars->config->get( 'video_default_height' ) );
    ...
    public function onEditMedia( $plugin, $media_item )
    {
        ...
        MediaManager::load( 'MediaManagerSelect' . $this->plain_name, $this->dot_path_prefix . 'video_jplayer.library.select', array( 'site'=>'site', 'type'=>'plugins', 'ext'=>'mediamanager' ) );
        ...
        foreach ($gallery_images as $image)
        {
            if (!in_array($image, $filenames_from_db))
            {
                ...
                if ($file->save())
                {
                    ...
                    if ($mediafile->save())
                    {
                        // and create the corresponding row in the video_jplayer table
                        $videojplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                        $videojplayer_table->media_id = $media_item->media_id;
                        $videojplayer_table->mediafile_id = $mediafile->mediafile_id;
                        if (!$videojplayer_table->save())
                        {
                            JFactory::getApplication()->enqueueMessage( $videojplayer_table->getError(), 'notice' );
                        }
                    }
                        else
                    {
    ...
    public function onAfterSaveMedia( $media_item )
    {
		...
		$params->set('videojplayer_auto_play', $post['videojplayer_auto_play']);
		$params->set('videojplayer_width', $post['videojplayer_width']);
		$params->set('videojplayer_height', $post['videojplayer_height']);
		$params->set('videojplayer_controlbar_position', $post['videojplayer_controlbar_position']);
		$params->set('videojplayer_show_icons', $post['videojplayer_show_icons']);
		$params->set('videojplayer_buffer_length', $post['videojplayer_buffer_length']);
		$params->set('videojplayer_loop', $post['videojplayer_loop']);
        ...
        if (!empty($item_remote_new))
        {
            ...
            if ($file->save())
            {
                ...
                if ($mediafile->save())
                {
                    // and create the corresponding row in the video_jplayer table
                    $videojplayer_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                    $videojplayer_table->load( array( 'mediafile_id'=>$mediafile->mediafile_id, 'media_id'=>$media_item->media_id ) );
                    $videojplayer_table->media_id = $media_item->media_id;
                    $videojplayer_table->mediafile_id = $mediafile->mediafile_id;
                    if (!$videojplayer_table->save())
                    {
