...
class plgMediaManagerSlideshow_Pikachoose extends MediaManagerPluginBase
{
	public $_element = 'slideshow_pikachoose';	
	public $dot_path_prefix = 'slideshow_pikachoose.';
	public $path_prefix = 'slideshow_pikachoose/';
	public $plain_name = 'SlideshowPikachoose';
	public $full_path = '';
	public $relative_path = '';
	
	function plgMediaManagerSlideshow_Pikachoose( &$subject, $config )
	{
    ...
    public function onEditMedia( $plugin, $media_item )
    {
        ...
        MediaManager::load( 'MediaManagerSelect' . $this->plain_name, $this->dot_path_prefix . 'slideshow_pikachoose.library.select', array( 'site'=>'site', 'type'=>'plugins', 'ext'=>'mediamanager' ) );
        ...
        foreach ($gallery_images as $image)
        {
            if (!in_array($image, $filenames_from_db))
            {
                ...
                if ($file->save())
                {
                    ...
                    if ($mediafile->save())
                    {
                        // and create the corresponding row in the slideshow_pikachoose table
                        $slideshowpikachoose_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                        $slideshowpikachoose_table->media_id = $media_item->media_id;
                        $slideshowpikachoose_table->mediafile_id = $mediafile->mediafile_id;
                        if (!$slideshowpikachoose_table->save())
                        {
                            JFactory::getApplication()->enqueueMessage( $slideshowpikachoose_table->getError(), 'notice' );
                        }
                    }
                        else
                    {
    ...
    public function onAfterSaveMedia( $media_item )
    {
        ...
        $params->set('slideshowpikachoose_include_jquery', $post['slideshowpikachoose_include_jquery']);
		$params->set('slideshowpikachoose_show_caption', $post['slideshowpikachoose_show_caption']);
		$params->set('slideshowpikachoose_autoplay', $post['slideshowpikachoose_autoplay']); 
		$params->set('slideshowpikachoose_auto_play_delay', $post['slideshowpikachoose_auto_play_delay']);
		$params->set('slideshowpikachoose_show_controls',$post['slideshowpikachoose_show_controls']);
		$params->set('slideshowpikachoose_slide_width', $post['slideshowpikachoose_slide_width']);
		$params->set('slideshowpikachoose_slide_height', $post['slideshowpikachoose_slide_height']);
		$params->set('slideshowpikachoose_container_width', $post['slideshowpikachoose_container_width']);
		$params->set('slideshowpikachoose_container_height', $post['slideshowpikachoose_container_height']);
		$params->set('slideshowpikachoose_loop', $post['slideshowpikachoose_loop']);
		$params->set('slideshowpikachoose_speed', $post['slideshowpikachoose_speed']);
        ...
        if (!empty($item_imageremote_new))
        {
            ...
            if ($file->save())
            {
                ...
                if ($mediafile->save())
                {
                    // and create the corresponding row in the slideshow_pikachoose table
                    $slideshowpikachoose_table = JTable::getInstance($this->plain_name,'MediaManagerTable');
                    $slideshowpikachoose_table->load( array( 'mediafile_id'=>$mediafile->mediafile_id, 'media_id'=>$media_item->media_id ) );
                    $slideshowpikachoose_table->media_id = $media_item->media_id;
                    $slideshowpikachoose_table->mediafile_id = $mediafile->mediafile_id;
                    if (!$slideshowpikachoose_table->save())
                    {
        ...
        if (!empty($item_local_new))
        {
            ...
            if ($file->save())
            {
                ...
                if ($mediafile->save())
                {
                    ...
                    $slideshowpikachoose_table->media_id = $media_item->media_id;
                    $slideshowpikachoose_table->mediafile_id = $mediafile->mediafile_id;
                    $slideshowpikachoose_table->enabled = '1';
                    if (!$slideshowpikachoose_table->save())
                    {
                        JFactory::getApplication()->enqueueMessage( $slideshowpikachoose_table->getError(), 'notice' );
                    }
