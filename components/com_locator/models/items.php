...
class LocatorModelItems extends LocatorModelBase {
 ...
 //   $query -> join('LEFT', '#__management_extensions AS e ON  e.ext_id = tbl.ext_id ');  
	...
	protected function _buildQueryFields(&$query)
	{
        ...
        $query -> select($this -> getState('select', 'tbl.item_id as id, tbl.name, tbl.name as title, tbl.latitude as lat, tbl.longitude as lng, tbl.notes as details, tbl.latitude, tbl.longitude, tbl.street, tbl.street2, tbl.city, tbl.state, tbl.zip, tbl.pin'));
        $query -> select($fields);
	...
	protected function prepareItem( &$item, $key=0, $refresh=false )
	{
			...
			$item->title = strtoupper(preg_replace('/[^\\/\-a-z\s]/i', '', $item->title)); //TODO  actually do this in the database so it is faster
            ...
            if(!empty($item->latitude) && !empty($item->longitude)) {
                 $dir[] = 'daddr='.$item->latitude.','.$item->longitude;
            }

			$item->directions = 'https://maps.google.com/?' .implode('&', $dir);

            if($item->id % 2) {
            $item->pin = 'media/com_locator/images/map/isis_icon.png';  
           }  else {
             $item->pin = 'media/com_locator/images/map/contactless.png';
           } 
             $item->nicedis = $this->formatDistance($item->distance);
             $item->address =  $this->makeAddress($item);
